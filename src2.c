...
void communicate(){
	...
	if(has_right_neighbour){
		position = 0;
		for(int j = cols + width; j < cols + 2*width; j++)
			for(int i = width; i < rows + width; i++)
	...
	if(has_bottom_neighbour){
		position = 0;
		for(int i = rows + width; i < rows + 2*width; i++)
...
void process(int i, int j)
{
	...
	/* processing left neighbours */
   	for(int _ = 1; _ <= width; _++)
		sum_neighbours += get_val(i, j - _, &nneighbours);

	/* processing right neighbours */
   	for(int _ = 1; _ <= width; _++)
		sum_neighbours += get_val(i, j + _, &nneighbours);
   	
	/* processing top neighbours */
	for(int _ = 1; _ <= width; _++)
		sum_neighbours += get_val(i - _, j, &nneighbours);
   	
	/* processing bottom neighbours */
	for(int _ = 1; _ <= width; _++)
		sum_neighbours += get_val(i + _, j, &nneighbours);
...
int main(int argc, char *argv[]) 
{
	...
	data = (double **)malloc((rows + 2*width)*sizeof(double*));
	temp = (double **)malloc((rows + 2*width)*sizeof(double*));
	for(int i=0; i<rows + 2*width; i++){
		data[i] = (double *)malloc((cols + 2*width)*sizeof(double));
		temp[i] = (double *)malloc((cols + 2*width)*sizeof(double));
	}

	/* initializing the matrix with random values */
	for(int i=width; i<rows + width + rows; i++)
		for(int j=width; j<cols + width; j++){
...
/* stencil communication + computation*/
	...
	/* debugging */
	FILE *file = fopen("output_actual.txt", "w");
