...
bool leader = 0; // whether the current run has heirarchical communication
bool this_is_leader = false; // current rank is leader
int my_leader; // leader of the current rank

void fill_has_neighbours()
{
	my_leader = (myrank %Px) * Px;
			...
			case(0):
				if (has_bottom_neighbour) {
					MPI_Gather(to_bottom, cols*width*sizeof(double), MPI_PACKED,
						   	...
						   	my_leader, MPI_COMM_WORLD);
					if (this_is_leader){ // leader
